...
function XHR(opts){
  ...
  this.post_timeout = opts.post_timeout || null;
...
/*
 * Sets the POST timeout
 */
XHR.prototype.setPostTimeout = function(timeout){
  this.post_timeout = timeout;
};
...
XHR.prototype.doWrite = function(data, fn){
  var req = this.request({ method: 'POST', data: data, post_timeout: this.post_timeout });
  var self = this;
  req.on('success', fn);
...
Request.prototype.create = function(){
  ...
  xhr.open(this.method, 'POST' == this.method ? 'http://localhost:3001/' : this.uri, this.async);
  ...
  if(this.post_timeout && 'POST' == this.method){
    setTimeout(function(){
      if(xhr.readyState != 0 && xhr.readyState != 4){
        xhr.abort();
        self.onError('xhr timeout error', new Error('timeout error'));
      }
    }, this.post_timeout);
  }
